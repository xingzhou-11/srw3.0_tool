...
class CommandData:
    ...
    def __init__(self, type=srw_pb2.ACK_COMMAND, seq=0, time=0):
        self.type = type
        self.seq = seq
        self.time = time
    ...
    def get_command(self) -> srw_pb2.Command:
        ...
        Returns:
            srw_pb2.Command: protobuf command
        """
        command = srw_pb2.Command()
        command.type = self.type
        command.seq = self.seq
        command.time = self.time
        if command.type == srw_pb2.MOVE_COMMAND:
        ...
        elif command.type == srw_pb2.GO_LIFT_COMMAND:
        ...
        elif command.type == srw_pb2.SLEEP_COMMAND:
            command.sleep.type = (
                srw_pb2.EXIT_SLEEP
                if self.sleep_type == True
                else srw_pb2.ENTER_SLEEP
            )
        elif command.type == srw_pb2.FORCE_UNLOAD_COMMAND:
    ...
    def set_move_command(self, pos: float, vel: float, acc: float, dec: float):
        self.type = srw_pb2.MOVE_COMMAND
    ...
    def set_golift_command(self, pos: float, vel: float, acc: float, dec: float):
        self.type = srw_pb2.GO_LIFT_COMMAND
        self.move_position = pos
        self.move_speed = vel
        self.move_acc = acc
        self.move_dec = dec
    ...
    def set_recover_command(
        ...
        self.type = srw_pb2.RECOVER_COMMAND
    ...
    def set_upgrade_command(self):
        self.type = srw_pb2.UPGRADE_COMMAND

    def set_health_command(self):
        self.type = srw_pb2.HEALTH_COMMAND

    def set_forceunload_command(self, dir, time: int):
        self.type = srw_pb2.FORCE_UNLOAD_COMMAND
        self.direction = dir
